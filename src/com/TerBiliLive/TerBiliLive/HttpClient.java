...
public class HttpClient {
	...
	/**
	 * 向指定 URL 发送GET 请求 并且返回头和内容
	 * @param url 发送请求的 URL
	 * @param param 请求参数
	 * @param cookie 请求所携带的Cookie
	 * @return 所代表远程资源的响应结果
	 */
	public static HttpClientEntity sendGetHeader(String url, String param, String cookie) {
		StringBuilder result = new StringBuilder();
		...
		try {
			...
			connection.connect();
//			 获取所有响应头字段
			httpClientEntity.setHeader(connection.getHeaderFields());
			// 遍历所有的响应头字段
//			for (String key : httpClientEntity.getHeader().keySet()) {
//				System.out.println("get响应头:"+key + "--->" + httpClientEntity.getHeader().get(key));
//			}
			// 定义 BufferedReader输入流来读取URL的响应
			in = new BufferedReader(new InputStreamReader(connection.getInputStream(), StandardCharsets.UTF_8));
			String line;
			...
			httpClientEntity.setResult(result.toString());
		} catch (Exception e) {
			System.out.println("发送GET请求出现异常！" + e);
			e.printStackTrace();
		}
	...
	/**
	 * 向指定 URL 发送POST 请求 并且返回头和内容
	 * @param url 发送请求的 URL
	 * @param paramMap 请求参数
	 * @param cookie 请求所携带的Cookie
	 * @return 所代表远程资源的响应结果
	 */
	public static HttpClientEntity sendPostHeader(String url, Map<String, String> paramMap, String cookie) {
		PrintWriter out = null;
		BufferedReader in = null;
		StringBuilder result = new StringBuilder();
		HttpClientEntity httpClientEntity = new HttpClientEntity();
		try {
			...
			out.flush();
			//			 获取所有响应头字段
			httpClientEntity.setHeader(conn.getHeaderFields());
			// 遍历所有的响应头字段
//			System.out.println("post响应头:--->" + httpClientEntity.getHeader().get("Set-Cookie"));
//			for (String key : httpClientEntity.getHeader().keySet()) {
//				System.out.println("post响应头:"+key + "--->" + httpClientEntity.getHeader().get(key));
//			}
			// 定义BufferedReader输入流来读取URL的响应
			in = new BufferedReader(new InputStreamReader(conn.getInputStream(), StandardCharsets.UTF_8));
			String line;
